...
%==============串并转换=============%
Rx_data_mult = reshape(r_mult,ifft_length,[]);
Rx_data_sig = reshape(r_sig,ifft_length,[]);
...
title_str=['单径信道 误码率为',num2str(ber_sig*100),'% 不加CP'];
title(title_str);
xlim([-2,2]);
ylim([-2,2]);
subplot(2,1,2);
...
title_str=['多径信道 误码率为',num2str(ber_mult*100),'% 不加CP'];
title(title_str);
xlim([-2,2]);
ylim([-2,2]);